...
function login(req, res) {
  let url =
    'https://fontmeme.com/permalink/200718/724a6d29d63468457272b3a6112fd84b.png';

  res
    .status(200)
    .render('pages/login', { route: '/login', linkToRoute: 'Login', url: url });
}
...
function test(req, res) {
  ...
  // accountAccess.splice(0, 1)
  // accountAccess.push(userName);
  accountAccess = req.body.userName;
...
function getPokeNames(req, res) {
  ...
  superagent
    ...
    .then((data) => {
      ...
      getPokeArr.sort();
      console.log(getPokeArr.sort());
      res.status(200).render('pages/show', {
        names: getPokeArr,
        route: '/favorites',
        linkToRoute: 'Favorites',
        url: url,
      });
    })
...
function addFavorites(req, res) {
  let saveToDB = [req.body.name, accountAccess];

  const SQL = `INSERT INTO $2 (name) VALUES ($1);`;
...
function getFavorites(req, res) {
  ...
  database
    ...
    .then((data) => {
      ...
      let url =
        'https://fontmeme.com/permalink/200718/62a4f2b98f2af5ac4721709ccb01b039.png';

      // res.status(200).render('pages/jQTest');
      res.status(200).render('pages/favorites', {
        names: row,
        route: '/',
        linkToRoute: 'Home',
        url: url,
